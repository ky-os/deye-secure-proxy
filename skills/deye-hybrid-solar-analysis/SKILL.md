...
Audit and optimize Deye hybrid (battery-backup) solar inverter setups for the user's own stations. This skill uses the **`deye-secure-proxy`** MCP — credentials are managed internally, no auth needed from the agent.
...
| Station | Station ID | Inverter SN | Battery | Work Mode |
|---|---|---|---|---|
| BaskSolar1-Atabay | 61688124 | 2507120169 | BATTERY_FIRST | ZERO_EXPORT_TO_LOAD |
| BaskSolar2-SalasReal | 62558309 | 2305202443 | 16S 52Ah LiFePO4, non-comm BMS | — |
...
No CAN/RS485 BMS communication ⇒ inverter estimates SOC from VOLTAGE, which mis-scales badly on LiFePO4's flat plateau. Reference for a **16S LiFePO4**:

- Nominal 51.2 V; full/top 56.5–58.4 V (3.55–3.65 V/cell); empty/cut 44–48 V (2.75–3.0 V/cell); safe charge ≤0.5C, discharge ≤1C.
- **If the app/API reports ~100% SOC at ~53 V (≈3.3 V/cell), the pack is really ~45–65% full** — voltage-SOC curve is misconfigured. Consequences: charger stops early (pack chronically ~half-charged), and SOC-based low/shutdown floors are NOT real protection. ONLY the voltage cutoffs (charge top + low-voltage cut ~46–48 V) protect the cells.
...
**Assumptions (state these to the user):**
- Nighttime window: ~14 hrs (6 PM – 6 AM) in the Philippines (no DST, PHT = UTC+8)
- Peak sun hours: ~5 hrs on a clear day
- Solar production: inverter_rated_W × 5 × 0.80 (inverter + wiring loss) ≈ kWh/day
- Nighttime load = battery's sole burden; daytime load covered by solar
- Safety factor: **1.25×** (aging, temperature, ramp)
- Usable DoD: **0.80** (20% floor)
...
**Reference table (PH residential, 5 kW inverter, 51.2V 16S LiFePO4):**
...
> Non-comm BMS caveat: SOC estimated from voltage mis-scales on LiFePO4's flat plateau. If the app shows 100% at ~53 V (≈3.3 V/cell), real pack is only 45–65% full. ONLY voltage cutoffs (charge top ≤58.4 V, low-voltage cut ~46–48 V for 16S) protect the cells.
...
## Bill-to-zero / net-metering reality (PH)

- "Return to the grid only what we consume" = net metering. In the Philippines (RA 9513 / ERC Resolution 09-2013, up to 100 kW) export credits are at the **wholesale/BGC (avoided-cost) rate, NOT retail** — so 1:1 kWh export-vs-import does NOT zero your bill unless you are ENROLLED in net metering. Unenrolled export is typically not credited (or flagged).
- **Without net metering, the only bill-zeroing lever is self-consumption + zero export**: every kWh consumed from solar/battery avoids the full retail rate. Zero-export + battery-first is already the right architecture. Residual bill = fixed monthly (distribution/connection) charges you cannot remove with any inverter.

## Consumption audit → "wattage to zero my bill"

1. Pull raw history: `get_device_history{deviceSn, startAt, granularity, measurePoints:[TotalConsumptionPower, TotalGridPower, TotalDCInputPower, BatteryPower], endAt}`. Use `get_device_measure_points` first to confirm available points.
...
See the `deye-open-mcp` skill for the full list of `action_type` values and allowed devices.
